...
function processCommand(command) {
    ...
    if (awaitingInteractionSelection) {
        ...
        if (!isNaN(selection) && selection >= 1 && selection <= currentInteractionOptions.length) {
            ...
            if (chosenOption === 'save') {
                saveGame();
                return;
            } else if (chosenOption === 'load') {
                loadGame();
                return;
            }
    ...
    const handler = commandHandlers[commandWord];
...
function saveGame() {
    ...
    printToOutput("Game progress saved.", "text-system");
}
        ...
        if (file) {
            ...
            reader.onload = e => {
                try {
                    ...
                    player = data;
                    printToOutput("Game progress loaded.", "text-system");
                    displayLocation();
                } catch (error) {
                    printToOutput("Error loading save file.", "text-danger");
                }
            };
